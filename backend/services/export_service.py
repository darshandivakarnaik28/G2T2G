"""
Dataset Export Service: Packages gestures, samples, raw landmarks, and ML-ready 128-feature arrays into a ZIP archive.
"""
import os
import csv
import json
...
def create_dataset_zip(db: Session) -> str:
    """Creates a complete ISL Dataset ZIP bundle with metadata and files"""
    init_storage()
    timestamp_str = datetime.datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    zip_filename = f"ISL_Dataset_Export_{timestamp_str}.zip"
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        # 2. samples.csv
        samples_csv_data = [
            ["sample_id", "gesture_id", "signer_id", "sample_type", "stored_file_path", "landmark_file_path", "frame_count", "fps", "detection_confidence", "duration", "created_at"]
        ]
        for s in samples:
            samples_csv_data.append([
                s.sample_id, s.gesture_id, s.signer_id, s.sample_type, s.stored_file_path,
        ...
        # 3. Gesture subfolders with media, landmarks & 128-feature ML arrays
        for s in samples:
            # Add media file
            if s.stored_file_path:
                full_media_path = BASE_DIR / s.stored_file_path
                if full_media_path.exists():
                    arc_path = f"ISL_Dataset/{s.gesture_id}/{'videos' if 'video' in s.sample_type.lower() else 'images'}/{full_media_path.name}"
            ...
            if s.landmark_file_path:
                ...
                if full_lm_path.exists():
                    arc_path = f"ISL_Dataset/{s.gesture_id}/landmarks/{full_lm_path.name}"
                    zf.write(full_lm_path, arc_path)

            # Add ML-ready 128-feature vector json
            ...
            if ml_feats:
                ...
                zf.writestr(f"ISL_Dataset/{s.gesture_id}/ml_features/{s.sample_id}_features_128.json", ml_json)
